...
serve(async (req) => {
  // corsHeaders is spread LAST everywhere: securityHeaders also defines
  // Access-Control-Allow-Headers (without x-turnstile-token) and the browser
  // drops the POST if the preflight doesn't allow every request header.
      ...
      if (turnstileSecret) {
        ...
        if (!turnstileData.success) {
          ...
          return new Response(
            ...
            { status: 403, headers: { ...securityHeaders, ...corsHeaders, "Content-Type": "application/json" } },
          );
        }
      }
    } catch (error) {
  ...
  try {
    ...
    if (!validation.success) {
      ...
      return new Response(
        ...
        { status: 400, headers: { ...securityHeaders, ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
    ...
    // Keep this cap in sync with MAX_QUESTIONS in src/pages/Chat.tsx.
    ...
    if (questionCount && questionCount >= 20) {
      return new Response(
        ...
        { status: 429, headers: { ...securityHeaders, ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
