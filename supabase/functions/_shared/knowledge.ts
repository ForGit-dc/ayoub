...
1. Opinion Science (opsci.ai) - Data Engineer, end-of-studies internship, May to Oct 2025 (6 months), Paris.
   ...
   Flagship case study (Twitter corpus on cloud technologies, September 2025): from 203,456 initial text segments, deduplication removed 15,242 exact duplicates (7.5% of the corpus), leaving 90,000 unique segments. Using embeddings of dimension 768 (all-mpnet-base-v2), UMAP (n_neighbors=30, min_dist=0.05, cosine metric) then HDBSCAN (min_cluster_size=100, min_samples=8), I obtained 142 thematic clusters covering 88% of the corpus (79,200 segments assigned; 12% noise = 10,800 segments), with a mean persistence of 0.62. Annotating 5 representatives per cluster (medoid plus 4) with GPT-4o validated 134 clusters (94%). Because only 5 representatives per cluster are annotated (about 710 across the 142 clusters) instead of every one of the 90,000 segments, this clustering-first approach drastically reduces the number of LLM calls, and therefore the cost and latency, versus annotating each segment directly. Stack used here: GCP (Cloud Functions, Cloud Tasks, BigQuery), PostgreSQL, Apache Kafka, Alembic, Python, UMAP, HDBSCAN, all-mpnet-base-v2, GPT-4o (OpenAI API).
