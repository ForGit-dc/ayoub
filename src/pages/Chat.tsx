...
export default function Chat() {
  ...
  const welcomeLine = useMemo(() => {
    ...
    if (visitorIdentity?.name) {
      return greeting.replace("Hi, I'm Ayoub", `Hello ${visitorIdentity.name}, I'm Ayoub`)
                     .replace("Bonjour, je suis Ayoub", `Bonjour ${visitorIdentity.name}, je suis Ayoub`)
                     .replace("مرحباً - أنا أيوب", `مرحباً ${visitorIdentity.name} - أنا أيوب`);
    }
    return greeting;
  ...
  const quickChips = useMemo(() => [{
    ...
    value: lang === 'fr' ? "Parle-moi de ton pipeline cloud chez Opinion Science." :
           lang === 'ar' ? "حدّثني عن خط أنابيبك السحابي في Opinion Science." :
           "Tell me about your cloud pipeline at Opinion Science."
  }, {
    label: t('quickChips').timeSeriesExample,
    value: lang === 'fr' ? "Comment le clustering a-t-il réduit tes coûts d'annotation LLM ?" :
           lang === 'ar' ? "كيف قلّص التجميع تكاليف التوصيف عبر LLM؟" :
           "How did clustering cut your LLM annotation costs?"
  }, {
    label: t('quickChips').currentWork,
    value: lang === 'fr' ? "Qu'as-tu construit chez Opinion Science ?" :
           lang === 'ar' ? "ماذا بنيت في Opinion Science؟" :
           "What did you build at Opinion Science?"
  }], [t, lang]);
  // Keep this cap in sync with the server-side check in supabase/functions/chat-ai/index.ts.
  ...
  useEffect(() => {
    const existing = localStorage.getItem("elyanboiy_chat_sid");
    const sid = existing || crypto.randomUUID();
    if (!existing) localStorage.setItem("elyanboiy_chat_sid", sid);
